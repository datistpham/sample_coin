...
import { getTeleBotTimerList } from 'src/redux/dashboard/account/action';
...
// import { getTeleBotTimerList } from '../../../redux/dashboard/account/action';
...
function ConfigurationList() {
  ...
  const botTimerList = useSelector((state) => state.teleBotTimerList);

  const handleDeleteConfiguration = async (_id) => {
    const response = await API_TELEBOT.deleteTimerById(_id);
    if (response.data.ok) {
      await getTimerList();
  ...
  const handleDeleteSelectedItem = async () => {
    ...
    await Promise.all(
      selectedItem.map(async (item) => {
        const response = await API_TELEBOT.deleteTimerById(item._id);
        if (response.data.ok) {
          current.count += 1;
    ...
    <>
      ...
      >
        <MenuItem
          ...
          onClick={() => {
            navigate.push(PATH_DASHBOARD.telebot.edit_timer(id));
            setOpen(null);
          }}
        >
