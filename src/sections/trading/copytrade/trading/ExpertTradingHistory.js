...
function ExpertTradingHistory(props) {
  ...
  const getHistory = async () => {
    setLoading(true);
    try {
      const response = await API_COPYTRADE.getHistory(EXPERT_TRADING_TYPE, page);
  ...
  const cancelListenSocket = () => {
    ...
    socket.current.off('update_history_trading', handleUpdateHistory);
  ...
  const deleteHistory = async () => {
    try {
      const response = await API_COPYTRADE.deleteHistory(EXPERT_TRADING_TYPE);
      if (response.status === 200) {
    ...
    try {
      ...
      dataHistory.forEach((history, index) => {
        const obj = {
          ...
          brokerUserName: history.brokerUserName,
          message: history.message,
          session: history.session,
          copyTradeSettingId: history.copyTradeSettingId,
        ...
        <Table className={classes.table} aria-label="simple table">
          ...
          >
            <TableRow>
              ...
              <TableCell style={{ color: theme.palette.mode === 'light' && 'black' }}>{translate('expert')}</TableCell>
              <TableCell style={{ color: theme.palette.mode === 'light' && 'black' }}>
                {translate('bet_type')}
              </TableCell>
          ...
          <StyledTableBody>
            ...
            {rows.map((row, index) => (
              ...
              >
                ...
                <TableCell>{row.brokerUserName}</TableCell>
                <TableCell>
                  <BetType row={row} />
                </TableCell>
...
const ExtraComponent = ({ row }) => {
  ...
  return (
    <TableCell colSpan={6}>
      <Typography>
        {translate('configuration_code')} : {row.copyTradeSettingId}
      </Typography>
      <Typography>
...
export default ExpertTradingHistory;
