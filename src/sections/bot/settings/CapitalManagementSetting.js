...
function CapitalManagementSetting({ isEdit, setValue, watch }) {
  ...
  return (
    <Grid container spacing={2} sx={{ mt: 3, mb: 3 }}>
      ...
      <Grid item xs={values.child_profit ? 4 : 12} md={values.child_profit ? 4 : 12}>
        <RHFSwitch name="child_profit" label={translate('child_profit')} />
      </Grid>
      {values.child_profit && (
        <Grid item xs={8} md={8}>
          <RHFTextField
            name="child_profit_amount"
            type={'text'}
            label={translate('child_profit_target')}
            size="small"
          />
        </Grid>
      )}
      <Grid item xs={values.child_loss ? 4 : 12} md={values.child_loss ? 4 : 12}>
        <RHFSwitch name="child_loss" label={translate('child_loss')} />
      </Grid>
      {values.child_loss && (
        <Grid item xs={8} md={8}>
          <RHFTextField name="child_loss_amount" type={'text'} label={translate('child_loss_target')} size="small" />
        </Grid>
      )}
