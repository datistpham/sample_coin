...
function MethodList() {
  ...
  useEffect(() => {
    setRows(methodList.filter((a) => a.type === 1));
  }, [methodList]);
  ...
  const columns = [
    ...
    },
    {
      ...
      maxWidth: 150,
      flex: 0.8,
      renderCell: (cellValues) => `${cellValues.row.data.conditions.length} ${translate('conditions')}`,
    },
  ];
        ...
        {details && !details.fromId && (
          <>
            <MenuItem
              ...
              onClick={() => {
                navigate.push(PATH_DASHBOARD.bot.edit_chain_method(id));
                setOpen(null);
              }}
            >
