...
import { getBotList, getBotTimerList } from 'src/redux/dashboard/account/action';
...
// import { getBotTimerList, getBotList } from '../../../redux/dashboard/account/action';
...
function Setting(props) {
  ...
  const updateTimer = async (data) => {
    const response = await API_BOT.updateTimer(_id, data);
    if (response.data.ok) {
      setEdit(false);
      getTimerList();
      reset();
      ...
      navigate.push(PATH_DASHBOARD.bot.timer, { replace: true });
  ...
  const createTimer = async (data) => {
    const response = await API_BOT.addTimer(data);
    if (response.data.ok) {
      // reset();
      getTimerList();
      enqueueSnackbar(translate('success'), { variant: 'success' });
  ...
  const getTimerInfo = async () => {
    ...
    try {
      const response = await API_BOT.getTimerById(_id);
