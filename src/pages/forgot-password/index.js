...
const ForgotPassword = () => {
  ...
  const handleSubmit =async (e) => {
    ...
    try {
      const response =
        step === 0 ? await API_AUTH.getRecoveryCodeForgotPass(userName) : await API_AUTH.postResetPassword(userName);
      if (response.data.ok) {
        enqueueSnackbar.enqueueSnackbar(response.data.m, { variant: 'success' });
        if (step === 0) {
          setStep(1);
      ...
      enqueueSnackbar.enqueueSnackbar(response.data.m, { variant: 'error' });
        ...
        >
          <BoxWrapper>
            ...
            <form noValidate autoComplete='off' onSubmit={handleSubmit}>
              <TextField onChange={(e)=> setUserName(e.target.value)} value={userName} autoFocus type='text' label='Username' sx={{ display: 'flex', mb: 4 }} />
              <Button fullWidth size='large' onClick={handleSubmit} variant='contained' sx={{ mb: 5.25 }}>
                Send reset link
              </Button>
